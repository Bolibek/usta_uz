import React from "react";

export default function Box({ box }) {
	const { name, contacts, field, profImg, workSamples, skills, badges } = box;

	return (
		<div className="w-full h-[15rem] mx-auto bg-white border-2 border-green-300 p-[1rem] rounded-md transform hover:scale-105 hover:z-50 ">
			<div className=" flex flex-row w-full relative">
				<div>
					<img
						className="w-[4rem] h-[4rem] rounded-full border-[0.1em]"
						src={profImg}
						alt="Profile image"
					/>
				</div>
				<div className="ml-5 text-xs">
					<h1 className=' font-bold'>{name}</h1>
					<div className='text-xs font-semibold'>{field}</div>
					<div className=" flex flex-row mb-1">
						<img className=" w-3 h-3" src="https://img.icons8.com/ios-glyphs/100/C9C9C9/star--v1.png"/>
						<img className=" w-3 h-3" src="https://img.icons8.com/ios-glyphs/100/C9C9C9/star--v1.png"/>
						<img className=" w-3 h-3" src="https://img.icons8.com/ios-glyphs/100/C9C9C9/star--v1.png"/>
						<img className=" w-3 h-3" src="https://img.icons8.com/ios-glyphs/100/C9C9C9/star--v1.png"/>
						<img className=" w-3 h-3" src="https://img.icons8.com/ios-glyphs/100/C9C9C9/star--v1.png"/>
					</div>
					<div className=" text-white">
						<span className=" bg-red-600 py-[0.1em] px-[0.4em] rounded-sm mr-2">7/24</span>
						<span className=" bg-green-600 py-[0.1em] px-[0.4em] rounded-sm">Tezkor so'rov</span>
					</div>
				</div>
				<div className="flex flex-col text-white absolute right-0 ">
          <img className=" cursor-pointer w-[2em] h-[2em] ml-0.5" src="https://img.icons8.com/external-others-inmotus-design/100/32AC49/external-Call-phone-operations-and-functions-others-inmotus-design-3.png"/>
          <img className=" cursor-pointer w-[2.3em] h-[2.3em]" src="data:image/png;base64,"/>
        </div>
			</div>
			<div className="my-2 text-xs">
        {skills.length ? skills.map((skill, index) => (
          <div key={index} className=" flex flex-row justify-between">
            <div>{skill.skill}</div>
            <div>+{skill.price},000 so'm</div>
          </div>
        )) : <h3>No skills mentioned</h3> }
        {workSamples.length && <span className="bg-blue-500 text-white px-1 cursor-pointer rounded-sm">Ko'proq...</span>}
      </div>

			<div className=" flex flex-row w-full h-[4rem] justify-between ">
        {workSamples.length ? workSamples.slice(0, 3).map((sample, index) => (
          <img className="w-[5.5rem] h-[3.3rem] rounded-md mt-2" src={sample} alt={`sample ${index}`} />
        )) : <h3>No photos uploaded</h3> }
      </div>
		</div>
	);
}
